...
df = pd.read_csv('./Datas/link_seoul_ver3_utf8.csv', encoding='utf8')
...
culture1 = pd.read_csv("POI/문화/공연장.csv", encoding='cp949', usecols=['위도', '경도'])
culture1 = culture1.values
culture = list()

if __name__ == "__main__":
    for data in culture1:
        mapping_culture = mapping_link(data[1], data[0])
        # print(mapping_culture)
        mapping_culture = indexToLink(mapping_culture)
        culture.append(mapping_culture)

    with open('list_to_mapping_culture1.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(culture)

    loc1 = (37.57017586019342, 126.98221099607764)
    idx = mapping_link(loc1[0], loc1[1])
    print(int(df.iloc[idx]['LINK_ID']))
